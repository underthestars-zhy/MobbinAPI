...
extension MobbinAPI {
    func fetchWorkspace() async throws -> Workspace {
        ...
        guard var URL = URL(string: "https://ujasntkfphywizsdaapi.supabase.co/rest/v1/workspaces") else { throw MobbinError.cannotFindToken }
        let URLParams = [
            "select": "name,id,type",
        ]
        ...
        let json = try JSON(data: data).arrayValue[0]

        let name = json["name"].stringValue
        let id = json["id"].stringValue
        let type = json["type"].stringValue

        return Workspace(name: name, id: id, type: type)
    }
}
